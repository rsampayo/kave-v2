...
async def test_remove_attachment_content_sqlite() -> None:
    ...
    # For SQLite with an already nullable column, we should just log and return
    # Verify execute was called once for the PRAGMA query
    mock_conn.execute.assert_called_once()
    # Commit should not be called in this case since we're just returning
    mock_conn.commit.assert_not_called()

    # Connect should be called with the connection URL
    mock_engine.connect.assert_called_once()
    # Execute should be called with the correct SQL command
    mock_conn.execute.assert_called_once()
